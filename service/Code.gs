...
function queryArxiv(id) {
  ...
  // The full category taxonomy is documented at https://arxiv.org/category_taxonomy.
  // Only use the first part of a compound category like astro-ph.CO.
  result['category'] = term.getValue().split('.')[0];
...
function doGet(e) {
  ...
  try {
    ...
    var files = yearFolder.getFilesByName(name);
    if(files.hasNext()) {
      // pdfSize == 0 indicates no bytes were downloaded.
      result['pdfSize'] = 0;
      result['fileLink'] = files.next().getUrl();
    }
    else {
      // Fetch the PDF file.
